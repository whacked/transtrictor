{
  description = "transtrictor environment";
  ...
  outputs = { self, nixpkgs, flake-utils, whacked-setup }:
    ...
    let
      ...
      arangodb = import ./nix/arangodb.nix { inherit pkgs; };
    in {
      devShell = whacked-helpers.mkShell {
        ...
        includeScripts = [
          ...
          (whacked-setup + /bash/postgresql_shortcuts.sh)
        ];
      } {
        buildInputs = [
          pkgs.gnumake
        ...
        '' + ''
          ...
          setup-couchdb-sample-init() {
              ...
              touch local.ini
              cat local.ini |
                initool s - couchdb single_node true |
                initool s - couchdb database_dir $COUCHDB_BASE_DIR/data |
                initool s - couchdb view_index_dir $COUCHDB_BASE_DIR/index |
                (
                    if [ "x$COUCHDB_AUTH_USERNAME" != "x" ]; then
                        initool s - admins "$COUCHDB_AUTH_USERNAME" "$COUCHDB_AUTH_PASSWORD"
                    else
                        cat
                    fi
                ) |
